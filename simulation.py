...
spawn = "BrainAgent5"     #the agent type spawned when clicking
min_population = 8        #auto spawns agents when population below min_population
...
while(run):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if mouse_pos[0]< sim_area_width and mouse_pos[1] < sim_area_height:
                dummy = eval(spawn)(sim_area_width, sim_area_height, [], archTypes,
                                    position=Position(mouse_pos[0],mouse_pos[1]))
    ...
    #claculate proximity to other agents
    for i, agent in enumerate(fullAgentList):
        agent.proximity(foodPlane, fullAgentList[i+1:])

    #iterate over all agents for main update
    for agent in fullAgentList:
        #check if new representative must be found (the one who's internals/brain are shown)
        ...
        # die (only living agents remain in the end)
        if agent.isAlive():
            alive.append(agent)
        else:
             deathCount += 1
    ...
    #============================================================ render agent names and archtype names ================
    #white backgrounds
    ...
    #blint agents and archtypes to window
    ...
    pygame.draw.rect(window, (0, 0, 0), (10, sim_area_height + 10, sim_area_width-180,
                                         win_height - sim_area_height - 20), 1)

    #draw graph
    if len(population_size_history)>1:
        pop_points = [(right_border-time+point[0], bottom_border-(point[1]*2))
                      for point in population_size_history]
        food_points = [(right_border-time+point[0], bottom_border-(point[1]*2)-1)
                       for point in food_supply_history]
        death_points = [(right_border - time + point[0], bottom_border - (point[1] * 2) + 1)
                        for point in evo_progress_history]
        #draw circles
        ...
        #aalines as alternative
        #pygame.draw.aalines(window, (0, 0, 200), False, pop_points)
        #pygame.draw.aalines(window, (255,140,0), False, food_points)
        #pygame.draw.aalines(window, (200, 170, 0), False, death_points)

    # draw labels/overlay
    window.blit(sys_font.render("Population size history", 0, (0, 0, 200)),
                (15, sim_area_height+15))
    window.blit(sys_font.render("Food supply history", 0, (255,140,0)),
                (right_border - 140, sim_area_height + 15))
    window.blit(sys_font.render("Evolutionary progess (avg death age/time)", 0, (200, 170, 0)),
                (270, sim_area_height + 15))
    for i in range(12):
        ...
        #window.blit(sys_font.render(str((i + 1) * 10)+"%", 0, (0, 0, 0)),
        # (sim_area_width - 32, win_height - 30 - i * 10 * 2))

    # draw statistics
    window.blit(sys_font.render("Avg death age: %d" % (int(avg_deathAge / 10)), 0, (0, 0, 0)),
                (right_border + 35, sim_area_height + 15))
    window.blit(sys_font.render("Deaths so far: %d" % (deathCount), 0, (0, 0, 0)),
                (right_border + 35, sim_area_height + 35))
    window.blit(sys_font.render("Time: %d" % (time / 12), 0, (0, 0, 0)),
                (right_border + 35, sim_area_height + 70))

    # ============================================================ render representative agent's internals/brain =======
    ...
    if at_representative and at_representative.dna.archTypeRef in archTypes:
        ...
        #    window.blit(sys_font.render("Right: %f" % (round(at_representative.rightEye_prox,3)), 0, (0, 0, 0)),
        # (right_border + 35, sim_area_height + 160))
        #    window.blit(sys_font.render("Left: %f" % (round(at_representative.leftEye_prox, 3)), 0, (0, 0, 0)),
        # (right_border + 35, sim_area_height + 175))
        #    window.blit(sys_font.render("Front: %f" % (round(at_representative.frontEarValue, 3)), 0, (0, 0, 0)),
        # (right_border + 35, sim_area_height + 190))
        #    window.blit(sys_font.render("Back: %f" % (round(at_representative.backEarValue, 3)), 0, (0, 0, 0)),
        # (right_border + 35, sim_area_height + 205))
